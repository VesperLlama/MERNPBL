...
export default function Bookings() {
  ...
  const PassengerForm = forwardRef(({ adults = 1, children = 0 }, ref) => {
    ...
    // errors keyed by `${index}.${field}`
    const [fieldErrors, setFieldErrors] = useState({});
    // touched tracking so we can show errors only after blur if desired
    ...
    const handleBlur = (index, field) => {
      ...
      const raw = (p[field] ?? "").toString().trim();

      let message = "";

      if (field === "name") {
        if (!raw) message = "Name is required";
        else if (!/^[A-Za-z]+(?: [A-Za-z]+)*$/.test(raw)) message = "Name must contain only letters and spaces";
      }

      if (field === "age") {
        if (!raw) message = "Age is required";
        else if (!/^\d+$/.test(raw)) message = "Age must be a positive integer";
        else {
          const n = Number(raw);
          if (n <= 0 || n > 120) message = "Enter a realistic age (1-120)";
        }
      }

      if (field === "gender") {
        if (!raw) message = "Gender is required";
      }

      if (message) {
        setFieldErrors((prev) => ({ ...prev, [`${index}.${field}`]: message }));
      } else {
        setFieldErrors((prev) => {
          const copy = { ...prev };
          delete copy[`${index}.${field}`];
    ...
    const validate = () => {
      ...
      passengers.forEach((p, i) => {
        ...
        if (!name) {
          ...
          errors.push({ index: i, field: "name", message: "Name must contain only letters and spaces" });
          newFieldErrors[`${i}.name`] = "Name must contain only letters and spaces";
        }
            ...
            <div className="pf-fields">
              <label className="pf-field">
                ...
                {fieldErrors[`${i}.name`] && (
                  <div
                    className="pf-err"
                    style={{ color: "red", fontSize: 12, marginTop: 3, fontWeight: 500 }}
                  >
                    {fieldErrors[`${i}.name`]}
                  </div>
                )}
